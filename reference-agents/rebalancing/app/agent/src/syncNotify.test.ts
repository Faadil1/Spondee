...
const URL = "http://127.0.0.1:9100/erc8183/job/955/response";
...
function fakeSigning(): SigningApi {
  return {
    ...
    clampPrice: () => 0n,
    signQuote: async () => ({}),
    verifySignedJob: async (jobId) => ({ ok: jobId === 955, reason: jobId === 955 ? "ok" : "wrong job", permanent: true }),
    jobSpec: async () => ({ task: "SPONDEE_TASK_B64_V1:test", terms: { price: "0" } }),
    submitResult: async (jobId) => ({ submitTx: TX, deliverableUrl: URL.replace("955", String(jobId)) }),
  };
}
...
test("default notify_funded remains asynchronous", async () => {
  ...
  const result = await seller.notifyFunded({ job_id: 955 });
