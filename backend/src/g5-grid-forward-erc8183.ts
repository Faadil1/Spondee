...
import { createPublicClient, decodeEventLog, getAddress, hexToString, http, type Hex, type Log } from "viem";
import {
  G5_GRID_COMMITMENT_PREFIX,
  G5_GRID_FEED,
  G5GridForwardAgentOutputSchema,
  assertForwardObservationAfterActivation,
  buildG5GridForwardCommitment,
  buildG5GridForwardPromise,
  decodeG5GridForwardCommitment,
  encodeG5GridForwardTask,
  evaluateForwardBaseline,
  type G5GridForwardAgentOutput,
  type G5GridForwardPromise,
...
const MAINNET_RPC = (env: NodeJS.ProcessEnv) => env.SPONDEE_G5_BSC_MAINNET_RPC?.trim() || "https://bsc-dataseed.bnbchain.org";
...
async function sendSkill(url: string, data: Record<string, unknown>, timeoutMs: number): Promise<Record<string, unknown>> {
  ...
  if (!response.ok) throw new Error(`G5 Grid seller HTTP ${response.status}`);
  const body = await response.json() as RpcReply;
  if (body.error) throw new Error(`G5 Grid seller error: ${body.error.message ?? "unknown"}`);
  const part = body.result?.parts?.[0]?.data;
  if (!part) throw new Error("G5 Grid seller returned no data part");
  return part;
}

function decodeCommitmentFromTerms(terms: unknown) {
  ...
  return decodeG5GridForwardCommitment(matches[0]);
}
...
async function fetchManifest(url: string, gatewayUrl: string): Promise<DeliverableManifest> {
  ...
  if (resolved.startsWith("file://")) throw new Error("G5 Grid forward proof refuses file:// deliverables");
...
async function verifyChainlinkRounds(rounds: G5ObservedRound[], env: NodeJS.ProcessEnv): Promise<void> {
  const client = createPublicClient({ transport: http(MAINNET_RPC(env), { timeout: 15_000 }) });
  if (await client.getChainId() !== 56) throw new Error("round verifier is not on BSC mainnet");
  const decimals = await client.readContract({ address: getAddress(G5_GRID_FEED), abi: feedAbi, functionName: "decimals" });
  for (const expected of rounds) {
    const r = await client.readContract({ address: getAddress(G5_GRID_FEED), abi: feedAbi, functionName: "getRoundData", args: [BigInt(expected.round_id)] });
    const [roundId, answer, , updatedAt] = r;
    const price = Number(answer) / 10 ** Number(decimals);
    const timestamp = new Date(Number(updatedAt) * 1000).toISOString();
    if (roundId.toString() !== expected.round_id) throw new Error(`Chainlink round id mismatch for ${expected.round_id}`);
    if (Math.abs(price - expected.price_usd) > 1e-8) throw new Error(`Chainlink price mismatch for ${expected.round_id}`);
    if (timestamp !== expected.updated_at) throw new Error(`Chainlink timestamp mismatch for ${expected.round_id}`);
  }
}

export interface G5GridForwardActivationResult {
  network: "bsc-testnet";
  ...
  promise: G5GridForwardPromise;
  funded_at: string;
  transactions: { create_job: string; register_job: string; set_budget: string; fund: string; submit: string };
  deliverable_url: string;
  deliverable_hash: string;
  agent_output: G5GridForwardAgentOutput;
  baseline_output: ReturnType<typeof evaluateForwardBaseline>;
  external_round_verification: "PASS";
  manifest_hash_verified: true;
  buyer_wallet_paid_wei: "0";
}

export async function runG5GridForwardActivation(
  task: G5GridForwardTask,
  env = process.env,
  onProgress: ProgressSink = () => undefined,
): Promise<G5GridForwardActivationResult> {
  if (env.SPONDEE_G5_GRID_FORWARD_EXECUTION_ENABLED !== "true") {
    throw new Error("G5 Grid forward execution gate is closed");
  }
  if (env.SPONDEE_LIVE_TESTNET_ENABLED !== "true") throw new Error("BSC testnet live gate is closed");
  if (env.BNBAGENT_USE_PAYMASTER !== "1") throw new Error("MegaFuel is required for the bounded G5 Grid pair");
  const sellerUrl = env.SPONDEE_G5_GRID_FORWARD_SELLER_URL?.trim();
  if (!sellerUrl) throw new Error("SPONDEE_G5_GRID_FORWARD_SELLER_URL is required");
  ...
  try {
    const taskDescription = encodeG5GridForwardTask(task);
    const previewRaw = await sendSkill(sellerUrl, { skill: "g5_grid_forward_preview", task_description: taskDescription }, 30_000);
    if (previewRaw.status !== "ok" || previewRaw.promise === null || typeof previewRaw.promise !== "object") throw new Error("seller rejected G5 forward Promise preview");
    const preview = previewRaw.promise as unknown as G5GridForwardPromise;
    const expectedPromise = buildG5GridForwardPromise(task, "0");
    if (sha256Hex(preview) !== sha256Hex(expectedPromise)) throw new Error("seller preview Promise differs from frozen task");

    const quoteRaw = await sendSkill(sellerUrl, {
      skill: "negotiate",
      task_description: taskDescription,
      terms: {
        deliverables: "Spondee G5 Grid forward observed agent output",
    ...
    if (quote.response.terms.price !== "0") throw new Error(`G5 forward proof refuses non-zero service price: ${quote.response.terms.price}`);
    if (quote.request.task_description !== taskDescription) throw new Error("signed G5 task description mismatch");
    const requestCommitment = decodeCommitmentFromTerms(quote.request.terms);
    const responseCommitment = decodeCommitmentFromTerms(quote.response.terms);
    if (!requestCommitment || !responseCommitment) throw new Error("signed quote lacks unique G5 forward commitment");
    if (JSON.stringify(requestCommitment) !== JSON.stringify(responseCommitment)) throw new Error("request/response G5 commitments differ");
    const expectedCommitment = buildG5GridForwardCommitment(expectedPromise);
    if (JSON.stringify(requestCommitment) !== JSON.stringify(expectedCommitment)) throw new Error("signed G5 commitment differs from preview Promise");
    ...
    const notify = await sendSkill(sellerUrl, { skill: "notify_funded", job_id: Number(jobId), wait_for_result: true }, (task.observation_rule.max_wait_seconds + 180) * 1000);
    ...
    while (Date.now() < deadline) {
      ...
      if (j.status === JobStatus.SUBMITTED) { finalStatus = "SUBMITTED"; break; }
      if (j.status === JobStatus.COMPLETED) { finalStatus = "COMPLETED"; break; }
    ...
    if (!manifest.verify(job.deliverable)) throw new Error("G5 deliverable manifest hash mismatch");
    let parsed: unknown;
    try { parsed = JSON.parse(manifest.response.content); } catch { throw new Error("G5 deliverable response is not JSON"); }
    const agentOutput = G5GridForwardAgentOutputSchema.parse(parsed);
    if (agentOutput.scenario_id !== task.scenario_id) throw new Error("agent output scenario mismatch");
    assertForwardObservationAfterActivation(task, agentOutput.rounds, fundedAt);
    await verifyChainlinkRounds(agentOutput.rounds, env);
    const baselineOutput = evaluateForwardBaseline(task, agentOutput.rounds);
