...
function Fail([string]$Message) {
    throw "[Spondee G4 Yield Live] $Message"
}
...
$agentRoot = Join-Path $repoRoot 'reference-agents\yield'
$agentDir = Join-Path $agentRoot 'app\agent'
$agentData = Join-Path $agentDir '.agent-data'
$taskPath = Join-Path $repoRoot 'reference-agents\yield\demo\g4-live-scenario.json'
...
$expectedSchema = 'spondee.yield.task.v1'
$expectedConclusion = 'SPONDEE_G4_YIELD_MEGAFUEL_LIVE_E2E_PASS'
$pnpmVersion = '10.24.0'
$agentPort = 9000
$bridgePort = 9100
...
$sellerOut = Join-Path $tempRoot "spondee-g4-yield-seller-$stamp.out.log"
$sellerErr = Join-Path $tempRoot "spondee-g4-yield-seller-$stamp.err.log"
$bridgeOut = Join-Path $tempRoot "spondee-g4-yield-bridge-$stamp.out.log"
$bridgeErr = Join-Path $tempRoot "spondee-g4-yield-bridge-$stamp.err.log"
$evidencePath = Join-Path $tempRoot "spondee-g4-yield-live-e2e-$stamp.json"

Write-Host 'Spondee G4 - Yield-only LIVE E2E' -ForegroundColor Green
Write-Host 'Health Factor job 949, Grid job 954, and Rebalancing job 955 are CLOSED.' -ForegroundColor Yellow
Write-Host 'This runner may create only one new Yield job. It must not touch prior-category jobs.' -ForegroundColor Yellow
Write-Host 'BSC Testnet only. Zero service price. MegaFuel primary. Stop-on-first-failure.'
Write-Host 'No Health Factor/Grid/Rebalancing execution. No mainnet, no user capital, no x402 payment, no final submission.' -ForegroundColor Yellow
...
try {
    ...
    if (-not (Test-Path $taskPath)) { Fail "Yield scenario missing: $taskPath" }

    $task = Get-Content $taskPath -Raw | ConvertFrom-Json
    if ($task.schema -ne $expectedSchema) {
        Fail "Yield scenario schema mismatch: $($task.schema)"
    }
    if ($task.scenario_id -ne 'spondee-yield-live-001') {
        Fail "Yield scenario id mismatch: $($task.scenario_id)"
    }
    if ($task.evidence_class -ne 'SIMULATION') {
        Fail "Yield evidence class must remain SIMULATION; received $($task.evidence_class)"
    }
    ...
    if (-not $SkipInstall) {
        ...
        Write-Step 'Installing frozen Yield Agent Studio workspace'
        Set-Location $agentRoot
        & npx --yes "pnpm@$pnpmVersion" install --frozen-lockfile
        if ($LASTEXITCODE -ne 0) { Fail 'Yield dependency installation failed.' }
    }
    ...
    Write-Step 'Starting Yield LocalStorage HTTP deliverable bridge'
    ...
    while ((Get-Date) -lt $bridgeDeadline) {
        if ($bridgeProcess.HasExited) {
            Tail-File $bridgeOut
            Tail-File $bridgeErr
            Fail "Yield deliverable bridge exited with code $($bridgeProcess.ExitCode)."
        }
        try {
    ...
    if (-not $bridgeReady) { Fail 'Yield deliverable bridge did not become healthy.' }
    Write-Host 'Local manifest bridge: HEALTHY' -ForegroundColor Green

    Write-Step 'Starting Yield seller with MegaFuel enabled'
    ...
    Write-Step 'Waiting for Yield seller /ping'
    $sellerReady = $false
    ...
    while ((Get-Date) -lt $sellerDeadline) {
        if ($sellerProcess.HasExited) {
            ...
            Tail-File $sellerErr
            Fail "Yield seller exited with code $($sellerProcess.ExitCode)."
        }
        try {
    ...
    if (-not $sellerReady) {
        Tail-File $sellerOut
        Tail-File $sellerErr
        Fail 'Yield seller did not become healthy within 75 seconds.'
    }
    Write-Host 'Yield seller A2A: HEALTHY' -ForegroundColor Green

    Write-Step 'Executing Yield Promise -> signed quote -> MegaFuel ERC-8183 -> verified Outcome Receipt'
    ...
    if (-not (Test-Path $evidencePath)) {
        ...
        Write-Host '--- seller stderr ---'
        Tail-File $sellerErr
        Fail "Yield live runner produced no public evidence file (exit=$liveExit)."
    }

    $evidence = Get-Content $evidencePath -Raw | ConvertFrom-Json
    if ($liveExit -ne 0 -or $evidence.conclusion -ne $expectedConclusion) {
        Write-Host "`nYield failed closed. Public evidence:" -ForegroundColor Red
        Get-Content $evidencePath | ForEach-Object { Write-Host $_ }
        Write-Host '--- seller stdout tail ---'
        Tail-File $sellerOut
        Write-Host '--- seller stderr tail ---'
        Tail-File $sellerErr
        Fail 'Yield did not pass. Do not retry blindly.'
    }

    if ($evidence.buyer_balance_before_wei -ne '0' -or $evidence.buyer_balance_after_wei -ne '0') {
        Fail 'Yield buyer zero-balance MegaFuel boundary was not preserved.'
    }
    if ($evidence.result.deliverable.manifest_hash_verified -ne $true) { Fail 'Yield manifest hash was not verified.' }
    if ($evidence.result.deliverable.spondee_receipt_verified -ne $true) { Fail 'Yield Outcome Receipt was not verified.' }
    if ($evidence.result.deliverable.receipt.evidence_class -ne 'SIMULATION') { Fail 'Yield receipt truth class changed unexpectedly.' }
    if ($evidence.observed_agent_advantage_claimed -ne $false) { Fail 'Yield incorrectly claimed observed Agent Advantage.' }

    Write-Host "`n============================================================" -ForegroundColor Green
    Write-Host 'SPONDEE G4 YIELD LIVE E2E: PASS' -ForegroundColor Green
    ...
    Write-Host 'This closes the G4 four-category live-transport sequence if recorded canonically.' -ForegroundColor Yellow
